...
### 쌓이는 기록은 언제 지우나 (retention)

`outbox_events`(발행분)와 `event_handled`(멱등 로그) 둘 다 지우지 않으면 무한히 쌓인다. "쌓이면 조회가 느려지지 않나"가 첫 걱정이지만, 그건 성능 문제가 아니다.

- outbox 폴러는 `published_at IS NULL`(미발행)만 조회 → **부분 인덱스**(`WHERE published_at IS NULL`)를 걸면 발행분이 아무리 쌓여도 인덱스 크기·조회 비용이 안 커진다.
- `event_handled` 조회는 `event_id` PK 기반 → 행이 늘어도 O(log N).
- 둘 다 B-tree라 행 수 증가가 조회 지연으로 이어지지 않는다. 삭제의 동기는 **조회 성능이 아니라 무한 증식 관리**(스토리지·백업/덤프·운영)다.

**결정적 차이 — 지워도 되는 시점이 다르다**

| 대상 | 성격 | 지워도 되는 시점 | 제약 |
|---|---|---|---|
| outbox 발행분 | 일 끝난 **죽은 데이터** | 발행 완료 즉시 (이력만 남기면 됨) | 없음 — 보관 기간 자유 |
| event_handled (멱등 로그) | **살아있는** 중복 판단 근거 | 같은 이벤트 재도착 가능성이 사라진 뒤 | **≥ Kafka 리텐션** |

멱등 로그를 너무 일찍 지우면, 같은 이벤트가 뒤늦게 재도착(재전송·리밸런싱·Outbox 재발행·DLT 재처리)했을 때 기록이 없어 **중복 처리**된다(likeCount·salesCount 이중 집계). "다시 올 수 있는 최대 창"의 상한은 Kafka 리텐션(기본 7일)이므로, `handled_at` 기준 **리텐션 + 여유 기간**이 지난 행만 삭제해야 안전하다.

**정리**

- 둘 다 "보관 후 배치 삭제"로 관리하되, outbox는 보관 기간이 **자유**(이력 필요만큼), 멱등 로그는 **Kafka 리텐션에 종속**된다.
- 숨은 결합: Kafka 리텐션을 늘리면 멱등 로그 보관 기간도 함께 늘려야 멱등이 깨지지 않는다.
- 증식 관리는 배치 삭제(필요 시 아카이빙/파티셔닝), 성능은 인덱스/PK로 이미 확보 — 둘을 분리해 접근한다.
