...
## 트랜잭션 경계를 합친 채로 유지해도 되는가 — 재검토

"트랜잭션 경계 분석"에서 외부 호출을 트랜잭션 안에 넣는 것을 위험하다고 지적했음에도, "트레이드오프 결정 — 구조 선택"에서는 결국 동일한 패턴(락 보유 중 PG 호출)을 선택지 1로 채택했다. 이게 "위험을 없앤 것"이 아니라 "위험의 크기를 캡핑한 것"이라는 점을 재검토했다.

### 발견 1. 캡핑의 근거였던 600ms 설정이 dev/qa/prd에 적용되지 않고 있었음

`resilience4j.circuitbreaker.instances.pg-simulator`, `resilience4j.timelimiter.instances.pg-simulator` (600ms) 설정이 `local, test` 프로파일 블록에만 선언돼 있었다. 결정 1(케이스 C)에서 고친 `spring.cloud.openfeign.circuitbreaker.enabled`는 전역으로 옮겼지만, 실제 임계치·타임아웃 값은 그대로 local/test 전용으로 남아 있었던 것 — 같은 종류의 프로파일 스코프 버그가 한 곳 더 있었다.

- 영향: dev/qa/prd에서는 Resilience4j 기본값(TimeLimiter 기본 1s, CircuitBreaker 기본 sliding-window 100 / wait-duration 60s)으로 동작 — "락 보유 시간은 최대 600ms로 제한된다"는 선택지 1의 핵심 근거가 해당 프로파일에서는 실제로 성립하지 않았다.
- **수정 완료.** `resilience4j:` 블록을 전역 `spring:` 블록으로 이동해 모든 프로파일에 600ms TimeLimiter와 CB 임계치가 적용되도록 했다. `pg.url`/`callback-url`은 환경별로 값이 달라야 하므로 `local, test` 블록에 유지했다.

### 발견 2. 커넥션 점유 위험의 실제 크기 — 현재 풀 설정 기준

"트랜잭션 경계 분석"에서 우려한 "커넥션 점유"는 두 가지로 분리해서 봐야 한다.

| 위험 | 영향 범위 | 현재 설정 기준 평가 |
|------|----------|---------------------|
| 락 대기 | **같은 주문**에 대한 동시 요청만 차단 | 케이스 E를 막기 위한 의도된 동작 — 문제 아님 |
| 커넥션 점유 | **모든** 결제 요청이 PG 응답까지 커넥션 1개씩 점유 | `maximum-pool-size: 40`(`modules/jpa/src/main/resources/jpa.yml:22`) 기준, 600ms 캡 안에서 **동시 40건 이상**이 겹쳐야 풀 고갈 — 현재 트래픽 규모에서는 발생 가능성이 낮음 |

결론: 선택지 1(트랜잭션 합침)을 유지해도, 600ms 캡이 전 프로파일에 실제로 적용되고 현재 풀 크기(40) 대비 동시 결제량이 그 이하인 한 위험은 감당 가능한 수준이다. 선택지 2(트랜잭션 분리)는 이 점유 시간을 0으로 줄이지만, B/D 케이스의 복구 API 필요성은 그대로 남고 "TX C(명시적 복구) 자체가 실패하면 영구 고착"이라는 새 실패 케이스가 추가되므로, 현재 규모에서는 추가 복잡도를 들일 이유가 없다고 판단해 **선택지 1 유지로 재확인**했다.
