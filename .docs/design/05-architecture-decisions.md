...
- 트랜잭션은 유스케이스 단위로 묶여야 한다.
  `재고 차감 + 주문 저장`이 하나의 원자 단위이고, 이를 조율하는 곳이 Facade다.
- Spring 기본 전파 방식(`REQUIRED`)으로 DomainService는 Facade 트랜잭션에 자연스럽게 참여한다.

---

## 결정 5. Value Object — Price, Stock

**결정**
- `Price`(가격)와 `Stock`(재고)를 `product/domain/`에 Java `record`로 분리한다.
- JPA Entity 필드(`Long price`, `Integer stock`)는 그대로 유지한다.
  VO는 검증·로직 경유 목적으로만 사용하고 `.value()`로 primitive를 꺼내 Entity에 저장한다.

**도입 이유**

VO 도입 전, 동일한 검증 로직이 생성자·`update()`·OrderItemModel 등 여러 곳에 분산되어 있었다.
...
// ProductModel.decreaseStock — VO 차감 후 primitive 저장
...
| 재고 음수 방지 | ProductModel에 산재 | `Stock` 자체가 불변으로 보장 |
...
**SoftDeletableEntity 분리 (미이행)**
- `BaseEntity`(id, createdAt, updatedAt)와 `SoftDeletableEntity`(+ deletedAt)를 분리할 계획이었으나,
  현재 `modules/jpa/BaseEntity`에 `deletedAt`이 포함되어 있어 모든 Entity가 불필요한 컬럼을 갖는다.
  향후 리팩토링 대상.
...
└── order/
    ...
    └── interfaces/  (OrderV1Controller, OrderV1Dto, AdminOrderV1Controller, ...)
```
