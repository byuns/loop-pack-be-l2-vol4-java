...
#### like_count 인덱스 제거 결정

`like_count`는 좋아요 등록/취소마다 UPDATE되는 컬럼이다.
인덱스가 있으면 매 UPDATE 시 B-Tree 노드도 갱신되므로 쓰기 비용이 증가한다.

**인덱스를 유지했을 때의 구조적 문제:**

좋아요 1회 → `idx_products_likes_desc` B-Tree 갱신 + `idx_products_brand_likes` B-Tree 갱신

인기 상품에 좋아요가 몰릴 때 같은 인덱스 페이지에 UPDATE가 집중된다.
InnoDB는 페이지 단위로 lock을 잡으므로 좋아요 요청들이 서로 대기하는 lock 경합이 발생한다.
"정렬 기준이면서 동시에 자주 바뀌는 값"에 B-Tree 인덱스를 두는 구조 자체가 맞지 않는다.

**like_count 정렬에 맞는 구조 — Redis Sorted Set(ZSET):**

```
좋아요 등록 → ZADD products:ranking {like_count} {productId}   # O(log N), 인메모리
좋아요 취소 → ZINCRBY products:ranking -1 {productId}

좋아요순 조회 → ZREVRANGE products:ranking 0 19               # 상위 20개 즉시 반환
```

- 쓰기: B-Tree 페이지 분할 없이 인메모리 갱신 → lock 경합 없음
- 읽기: 이미 정렬된 자료구조에서 범위 조회 → O(log N + K)
- MySQL 인덱스 갱신 자체가 사라짐

**현재 판단:**
Redis ZSET은 미구현 상태이므로 `likes_desc` 정렬은 캐시 미스 시 filesort로 fallback된다.
현재 트래픽 수준에서는 Redis 캐시(TTL 5분)가 대부분의 요청을 커버하므로 허용 가능한 수준이다.
좋아요 트래픽이 임계점을 넘는 시점에 Redis ZSET으로 전환한다.
...
`like_count` 인덱스는 쓰기 경합 문제로 제외. Redis ZSET으로 대체 예정 (미구현).
...
| `like_count` | 높음 (사용자 행동마다) | ❌ | B-Tree 쓰기 경합 → Redis ZSET이 적합 |
...
| 전체 최신순 | `ALL` | NULL | 99,449 | ✅ |
| 전체 가격순 | `ALL` | NULL | 99,449 | ✅ |
| 브랜드 + 좋아요순 | `ALL` | NULL | 99,516 | ✅ |
| 브랜드 + 최신순 | `ALL` | NULL | 99,449 | ✅ |
| 브랜드 + 가격순 | `ALL` | NULL | 99,449 | ✅ |

전체 풀스캔 + filesort. 정렬 조건 무관하게 동일하게 느림.
...
| 전체 최신순 | `ALL` | NULL | 99,449 | ✅ 여전히 발생 |
| 전체 가격순 | `ALL` | NULL | 99,449 | ✅ 여전히 발생 |
| 브랜드 + 좋아요순 | `ref` | idx_products_brand_likes | **9,116** | ❌ 제거 |
| 브랜드 + 최신순 | `ref` | idx_products_brand_likes | 10,180 | ✅ 여전히 발생 |
| 브랜드 + 가격순 | `ref` | idx_products_brand_likes | 10,180 | ✅ 여전히 발생 |

좋아요순은 해결됐지만 최신순·가격순은 여전히 문제. 브랜드 + 최신/가격은 brand_id 조건에 idx_products_brand_likes를 쓰지만 정렬 컬럼이 맞지 않아 filesort 잔존.

---

**3단계 — 최신/가격 인덱스 추가 + like_count 인덱스 제거** (최종)

like_count 인덱스는 쓰기 경합 문제로 제거. created_at·price 인덱스를 추가해 최신/가격순을 커버.

| 케이스 | type | key | rows | filesort |
|--------|------|-----|------|---------|
| 전체 좋아요순 | `ALL` | NULL | 99,516 | ⚠️ filesort (Redis 캐시 커버) |
| 전체 최신순 | `index` | idx_products_created_at | **20** | ❌ 제거 |
| 전체 가격순 | `index` | idx_products_price | **20** | ❌ 제거 |
| 브랜드 + 좋아요순 | `ref` | idx_products_brand_created_at | ~5,032 | ⚠️ filesort (Redis 캐시 커버) |
...
- [x] like_count 인덱스 제거 — B-Tree 쓰기 경합 문제, Redis ZSET으로 대체 예정
...
✅ like_count 인덱스 제거 → B-Tree 쓰기 경합 문제, Redis ZSET 대체 예정
✅ Redis 캐시 적용
```
