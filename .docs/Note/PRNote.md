...
### 문제 정의
- **현재 동작/제약**: 주문 흐름이 `createOrder → startPayment → confirmPayment` 3단계 API로 분리. 초기 설계에서 쿠폰 소비는 `createOrder`, 재고 선점은 `startPayment` 트랜잭션에서 각각 처리됨.
- **문제(리스크)**: `startPayment`가 실패하면 `createOrder`에서 이미 커밋된 쿠폰 소비가 롤백되지 않아 쿠폰이 영구 소진됨. 동시 주문 시 동일한 쿠폰·재고 row에 여러 스레드가 동시에 접근하면 Lost Update 발생.
