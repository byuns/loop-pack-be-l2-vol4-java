...
retry를 도입하면서 FallbackFactory를 두 번 고쳐야 했다. 처음에는 PG 호출이 어떤 이유로 실패하든 `SERVICE_UNAVAILABLE`을 던지면 그만이었다. retry 코드를 추가하면서 "재시도할 수 있는 실패인가, 아닌가"를 구분해야 한다는 걸 알았다. CB가 열린 상태(`CallNotPermittedException`)는 PG 자체가 다운된 것이므로 재시도해도 CB가 또 막는다. 타임아웃·네트워크·PG 500은 일시적 실패라 기다렸다가 재시도하면 성공할 수 있다. 두 경우를 모두 `SERVICE_UNAVAILABLE`로 뭉쳐두면 retry 코드에서 구분할 방법이 없어서, 일시적 실패만 `PgRetriableException`으로 분리하고 CB OPEN은 즉시 차단하도록 바꿨다.
